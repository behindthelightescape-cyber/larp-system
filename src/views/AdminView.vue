...
onMounted(async () => {
  ...
  supabase.auth.onAuthStateChange((_event, _session) => {
    ...
    if (_session) loadDashboardStats()
  })

  if (session.value) loadDashboardStats()
...
<template>
  <div v-if="isLoading" class="admin-layout center-content">
    ...
    <h2 style="color: #D4AF37; margin-top: 20px;">機密區域連線中...</h2>
  </div>
  ...
  <div v-else class="admin-wrapper">
    ...
    <main class="main-content">
        ...
        <div style="display: flex; align-items: center; gap: 15px;">
          ...
          <h2 class="page-title">
            {{ 
              ...
              currentTab === 'session' ? '場次大廳 (掃碼監控)' :  // 🚀 加這行
      ...
      <div v-show="currentTab === 'game'" class="panel active">
        <GameManager @update-stats="loadDashboardStats" />
      </div>
...
.admin-layout { 
  ...
  z-index: 9999; background-color: #050505; color: white; font-family: 'Segoe UI', sans-serif; 
}
...
.admin-wrapper { 
  position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; 
  z-index: 9999; display: flex; background-color: #0a0a0a; color: white; overflow: hidden; 
}
