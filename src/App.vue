...
<template>
  ...
  <div class="app-wrapper">
    <div class="app-container">
      <div v-if="loading" class="loading-screen"><div class="loader"></div><p>讀取角色資料...</p></div>
...
.content-area { flex: 1; overflow-y: auto; padding: 25px; padding-bottom: 120px; }
...
.card-main { display: flex; padding: 15px; gap: 15px; }
.thumb-col { width: 80px; height: 80px; border-radius: 10px; overflow: hidden; flex-shrink: 0; border: 1px solid #444; background: #000; }
.script-thumb { width: 100%; height: 100%; object-fit: cover; }
.card-info-content { flex: 1; display: flex; flex-direction: column; justify-content: center; }
.header-row { margin-bottom: 6px; }
.script-title { margin: 0; font-size: 1.2rem; color: #fff; font-weight: bold; line-height: 1.3; }
.gm-row { margin-bottom: 8px; font-size: 0.95rem; color: #ccc; display: flex; align-items: center; gap: 5px; }
.gm-label { color: #666; font-size: 0.8rem; text-transform: uppercase; }
.gm-value { font-weight: 500; }
.exp-row { display: flex; justify-content: space-between; align-items: center; }
.exp-badge { color: var(--primary); font-size: 1rem; font-weight: bold; }
...
.avatar-wrapper { width: 160px; height: 160px; border-radius: 50%; border: 4px solid rgba(255, 215, 0, 0.5); padding: 5px; box-shadow: 0 0 35px rgba(0,0,0,0.6); background: rgba(0,0,0,0.3); backdrop-filter: blur(5px); z-index: 2; overflow: hidden; }
...
.display-name { font-size: 2.2rem; font-weight: 800; color: #fff; margin: 0; text-shadow: 0 2px 10px rgba(0,0,0,0.5); }
...
.stats-grid { display: flex; justify-content: space-around; align-items: center; padding: 30px 0; margin-bottom: 30px; }
.stat-item { display: flex; flex-direction: column; align-items: center; width: 30%; }
.stat-val { font-size: 2.2rem; font-weight: 800; color: #fff; margin-bottom: 8px; }
...
/* 🔥 底部導航修正 (縮小 + 單色) 🔥 */
.bottom-nav-glass {
  position: fixed; bottom: 20px; /* 貼低一點 (原本 30) */
  left: 50%; transform: translateX(-50%);
  width: calc(var(--card-width) - 40px);
  height: 60px; /* 變矮 (原本 70) */
...
.nav-item {
  width: 40px; height: 40px; /* 觸控區縮小 (原本 50) */
...
/* settings & ticket styles (kept from prev version) */
